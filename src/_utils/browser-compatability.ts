export default function assertEnvironmentCompatibility() {
  ...
  function isDenoEnv(
    obj: unknown
  ): obj is { Deno: { version: { deno: string } } } {
  ...
  const throwUnsupported = (
    ...
    window.alert(
      `${browser} version ${current} is not supported.\n` +
        `Please update your browser or switch to a modern, compatible browser.\n` +
        `Minimum required version is ${required}.`
    );

    throw new Error(
      `${browser} version ${current} is not supported. Please upgrade to at least version ${required}.`
    );
  };
  ...
  if (isSafari) {
    ...
    if (version < 18.5) throwUnsupported("Safari", version.toString(), "18.5");
    return;
  }
  ...
  if (chromeMatch && ua.includes("Chrome") && !ua.includes("Edg")) {
    ...
    if (version < 113) throwUnsupported("Chrome", version.toString(), "113");
    return;
  }
  ...
  if (firefoxMatch) {
    ...
    if (version < 110) throwUnsupported("Firefox", version.toString(), "110");
    return;
  }
  ...
  if (edgeMatch) {
    ...
    if (version < 113) throwUnsupported("Edge", version.toString(), "113");
    return;
  }

  throw new Error(
    "Unsupported or unknown browser. Please update or switch to a compliant modern browser."
  );
}

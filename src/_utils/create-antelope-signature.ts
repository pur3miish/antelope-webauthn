...
export default async function createAntelopeSignature(
  assertionResponse: CreateAntelopeSignatureInput,
  ...
  const public_key_bytes = base58_to_binary(
    antelope_public_key.replace("PUB_WA_", "")
  );
  const clientDataHash = await sha256(new Uint8Array(clientDataJSON));
  const hash = await sha256(
    Uint8Array.from([...new Uint8Array(authenticatorData), ...clientDataHash])
  );

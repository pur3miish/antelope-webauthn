...
export default async function createAntelopeWebAuthnPublicKey({ attestationObject, clientDataJSON, }) {
    ...
    const checksum = await ripemd160(Uint8Array.from([...webauthn_public_key, 87, 65]) // 87, 65 -> WA ascii
    );
    const antelope_public_key = "PUB_WA_" +
        binary_to_base58(Uint8Array.from([...webauthn_public_key, ...checksum.slice(0, 4)]));
